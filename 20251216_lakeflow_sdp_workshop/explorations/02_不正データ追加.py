...
def generate_users_insert(num_users=100):
    ...
    max_last_updated = (existing_users_sdf.agg({"last_updated": "max"}).collect()[0][0]).strftime("%Y-%m-%d")
    ...
    return (
        ...
        .withColumn("random_date", expr(f"date_add(date('{max_last_updated}'), CAST(rand() * 30 AS INTEGER))"))
...
def get_last_updated_range():
    # existing_users['last_updated']の最大値を取得し、その翌日から30日間の範囲を返す
...
def generate_transactions(users, products, num_transactions=10000):
    ...
    existing_transactions_sdf = spark.table(f"{CATALOG_NAME}.{SCHEMA_NAME}.transactions")
    max_transaction_date = (existing_transactions_sdf.agg({"transaction_date": "max"}).collect()[0][0]).strftime("%Y-%m-%d")

    new_user_ids = [row.user_id for row in users.select("user_id").collect()]
    transactions = (
        spark.range(1, num_transactions + 1).withColumnRenamed("id", "transaction_id")
        .withColumn("user_id", expr(
        f"element_at(array({', '.join([str(v) for v in new_user_ids])}), cast(floor(rand() * {len(new_user_ids)}) + 1 as int))"))
        .withColumn("product_id", expr(f"floor(rand() * {products.count()}) + 1"))
        .withColumn("quantity", when(rand() < 1.0 / 3, 0).otherwise(floor(rand() * 6) + 1))
        .withColumn("store_id", round(rand() * 10 + 1))
        .withColumn("random_date", expr(f"date_add(date('{max_transaction_date}'), CAST(rand() * 30 AS INTEGER))"))
        .withColumn("month", date_format("random_date", "M").cast("int"))
        .withColumn("is_weekend", dayofweek("random_date").isin([1, 7]))
        .withColumn("transaction_date",
            when((rand() < 0.1) & ((expr("month") == 8) | (expr("month") == 12)), expr("random_date"))
            .when((rand() < 0.1) & expr("is_weekend"), expr("random_date"))
            .otherwise(expr(f"date_add(date('{max_transaction_date}'), CAST(rand() * 30 AS INTEGER))"))
        )
        .drop("random_date", "month", "is_weekend")
    )
...
transactions = generate_transactions(users_insert, existing_products_sdf)
...
tx_with_region = transactions.join(users_insert.select("user_id", "region"), on="user_id", how="inner")
...
# 顧客情報の書き出し (CSV)
users_final.toPandas().to_csv(f'{users_backfill_dir}users_{random_suffix}.csv', index=False, encoding='utf-8-sig')
