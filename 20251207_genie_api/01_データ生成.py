...
JAPANESE_FIRST_NAMES = [
    "翔太", "大輔", "健太", "陽介", "拓海", "優", "誠", "直樹", "悠真", "亮",
    "美咲", "結衣", "彩香", "愛", "萌", "優花", "紗季", "舞", "真由", "菜々子"
]

def generate_japanese_name():
    last = random.choice(JAPANESE_LAST_NAMES)
    first = random.choice(JAPANESE_FIRST_NAMES)
    return f"{last} {first}"

generate_japanese_name_udf = udf(generate_japanese_name, StringType())
...
def generate_users(num_users=10000):
    ...
    return (
        ...
        # ▼ 日本人氏名を付与
        .withColumn("name", generate_japanese_name_udf())
        ...
        # ▼ メールは一意ID形式に
        .withColumn("email", concat(lit("user"), col("user_id"), lit("@example.com")))
...
# --------------------------------------------------
# 商品名をカテゴリに合わせて上書き生成する仕組み
# --------------------------------------------------
NAMES_BY_SUBCAT = {
    # 食料品
    "野菜": ["トマト", "きゅうり", "にんじん", "玉ねぎ", "じゃがいも", "ほうれん草", "ブロッコリー", "なす"],
    "果物": ["りんご", "バナナ", "オレンジ", "いちご", "ぶどう", "みかん", "キウイ", "パイナップル"],
    "健康食品": ["プロテインバー", "マルチビタミン", "オメガ3サプリ", "コラーゲンパウダー", "腸活タブレット"],
    "肉類": ["鶏むね肉", "豚ロース", "牛肩ロース", "合挽きミンチ", "ベーコン", "ソーセージ"],

    # 日用品
    "キッチン用品": ["フライパン", "包丁", "まな板", "計量カップ", "保存容器（耐熱）", "スライサー", "ピーラー"],
    "スポーツ・アウトドア用品": ["トレッキングポール", "テント（2人用）", "寝袋（3シーズン）", "LEDランタン", "ヨガマット", "トレイルランニングシューズ", "ハイドレーションボトル"],
    "医薬品": ["総合感冒薬", "解熱鎮痛薬", "鼻炎薬", "胃腸薬", "目薬", "絆創膏（滅菌）", "湿布薬"],
    "冷暖房器具": ["セラミックヒーター", "サーキュレーター", "加湿器", "卓上扇風機", "電気毛布", "ポータブルクーラー"]
}

def _product_name_by_cat_py(category: str, subcategory: str) -> str:
    """カテゴリ/サブカテゴリに応じて商品名を返す（既存generate_productnameを尊重しつつ上書き）"""
    pool = NAMES_BY_SUBCAT.get(subcategory)
    if pool:
        base = random.choice(pool)
        suffix = random.choice(["", " S", " M", " L", " Pro", " Plus"])
        return f"{base}{suffix}".strip()
    return generate_productname()  # 想定外は既存ランダム生成

generate_productname_by_cat_udf = udf(_product_name_by_cat_py, StringType())
...
def generate_products(num_products=100):
    ...
    - product_name: ランダムな商品名（カテゴリに合わせて上書き）
    ...
    return (
        ...
        .withColumn("product_name", generate_productname_udf())  # ← 元のランダム生成を残す
        ...
        # ▼ ここでカテゴリ別に日本語の商品名を上書き
        .withColumn("product_name", generate_productname_by_cat_udf(col("category"), col("subcategory")))
        .withColumn("price", round(rand() * 1000 + 100, 2))  # ← 元ロジック維持
        # ▼ サブカテゴリごとに価格帯を“寄せる”軽いバイアス（範囲は100〜1100のまま）
        .withColumn(
            "price",
            when(col("subcategory") == "野菜", round(rand() * (450-120) + 120, 2))
            .when(col("subcategory") == "果物", round(rand() * (650-150) + 150, 2))
            .when(col("subcategory") == "肉類", round(rand() * (900-350) + 350, 2))
            .when(col("subcategory") == "健康食品", round(rand() * (1000-450) + 450, 2))
            .when(col("subcategory") == "キッチン用品", round(rand() * (900-300) + 300, 2))
            .when(col("subcategory") == "スポーツ・アウトドア用品", round(rand() * (1100-800) + 800, 2))
            .when(col("subcategory") == "医薬品", round(rand() * (1000-500) + 500, 2))
            .when(col("subcategory") == "冷暖房器具", round(rand() * (1100-900) + 900, 2))
            .otherwise(col("price"))
        )
        .withColumn("stock_quantity", round(rand() * 100 + 1))  # ← 元ロジック維持
        .withColumn("cost_price", round(col("price") * 0.7, 2))  # ← 元ロジック維持
...
today_str = datetime.now().strftime("%Y-%m-%d") # 例: '2025-09-05'
...
def generate_transactions(users, products, num_transactions=1000000):
    ...
    transactions = (
        ...
        .withColumn(
            "transaction_date",
            expr(f"date_add(date('{today_str}'), -CAST(rand() * 365 AS INTEGER))")
        )
...
#     dest=f"/Volumes/{catalog}/{schema}/{volume_csv}",
#     owner="komae5519pv",
#     repo="komae_dbdemos",
...
# csv_path = f"/Volumes/{catalog}/{schema}/{volume_csv}/gold_feedback.csv"  # ファイル名まで
# df = (
